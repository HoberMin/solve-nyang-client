...
const CatCard = ({ name }: CatCardProps) => (
  <div className='text-center'>
    ...
    <p className='mt-3 text-sm font-medium text-white'>{getCatKorName(name)}</p>
    <span className='text-xs text-[#26ffc9]'>H등급</span>
  </div>
);
...
const ResultDisplay = ({ name }: { name: string }) => {
  return (
    <div className='text-center'>
      ...
      <p className='mt-4 text-xl font-bold text-white'>{getCatKorName(name)}</p>
...
const ValentineEventPage = () => {
  const { data } = useGetValentineCoin();
  const { coin } = data;
  const getValentineAvatar = useGetValentineCharacter();
  const [isShowAnimation, setShowAnimation] = useState(false);
  const [isShowConfetti, setShowConfetti] = useState(false);
  const [avatarCat, setAvatarCat] = useState<null | string>(null);
  const isProcessingRef = useRef(false);

  const useChocoCoin = async () => {
    if (coin <= 0 || isProcessingRef.current) {
      if (coin <= 0) {
    ...
    const { avatarName } = await getValentineAvatar();
    setAvatarCat(avatarName);
            ...
            <div className='rounded-2xl bg-slate-800/50 p-6 backdrop-blur-sm'>
              ...
              <div className='grid grid-cols-3 gap-6'>
                {VALENTINE_CATS.map(cat => (
                  <CatCard key={cat.name} name={cat.name} />
                ))}
              </div>
            </div>
          ...
          <div className='flex flex-col items-center justify-between rounded-2xl bg-slate-800/50 p-8 backdrop-blur-sm'>
            ...
            <div className='w-full space-y-4'>
              ...
              <button
                ...
                disabled={coin <= 0 || isShowAnimation}
